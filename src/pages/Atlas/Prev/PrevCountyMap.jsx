...
import { EditableMarkers } from "./Prev";
...
export function AtlasMap({
  ...
  function zoomToFeature(e, feature) {
    ...
    const uri = `${url}${rest}${isSetup ? `&token=${token}` : portal ? `&ref=${ref}&role=${role}` : ""
      }`;
    map.fitBounds(e.target.getBounds());
    push(uri);
  }
  ...
  function style(feature) {
    ...
    return {
      fillColor: ovverrideColor || atlas
        ? getAtlasColor()
      ...
      weight: 2,
      opacity: 1,
      color: settings ? errorColor : "white",
      dashArray: "3",
      fillOpacity: 0.7,
    };
  }
...
export function PeopleMap({ title, dat, checked, Icons, Icon, event }) {
  const map = useMap();
                      ...
                      <Popup>
                        {!event && (
                          <Box>
                            ...
                            <Typography>{p?.role}</Typography>
                            <Button
                              onClick={() => push(`/operations/Notifications/`)}
                            >
                              Send message
                            </Button>
                        ...
                        {event && (
                          <Box>
                            ...
                            <Typography>
                              {p?.allday ? "All day" : null}
                            </Typography>
                            <Typography>Starts: {p?.start}</Typography>
                            <Typography>Ends: {p?.end}</Typography>
                          </Box>
                        )}
                      </Popup>
                    </Marker>
          ...
          <React.Fragment>
            {dir?.map((p, i) => {
              ...
              return (
                <div key={i}>
                  {p?.geom && (
                    <EditableMarkers coordinates={getPosition(p)} p={p} />
                  )}
                </div>
              );
            })}
