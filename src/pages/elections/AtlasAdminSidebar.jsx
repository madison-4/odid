...
export const PeoplesIcons = {
  Search: <SearchIcon operations sx={{ color: "gray" }} />,
...
export default function MiniDrawer({ children, area }) {
  const { officials, messages, dispatch, resources } =
  ...
  return (
    <Box sx={{ display: "flex" }}>
      ...
      <Drawer variant="permanent" open={false}>
        ...
        <SearchDialog operations />
        <List>
          {[
            area === "operations" ? "Search" : null,
            "Officials",
            "Coordinators",
            "Members",
            area === "portal" ? "Votes" : null,
          ]
            .filter((f) => Boolean(f))
            .map((text, index) => (
              <ListItem
                ...
                <Badge
                  ...
                  badgeContent={
                    index === 0
                      ? unReadEvents?.length
                      : index === 1
                        ? resources?.filter((f) => f.isNew)?.length
                        : 0
                  }
                >
                  {ResIcons[text]}
